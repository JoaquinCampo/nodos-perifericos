...
import { updateHealthWorkerAction } from "~/server/actions/health-worker";
import {
  type UpdateHealthWorkerSchema,
  updateHealthWorkerSchema,
} from "~/server/schemas/health-worker";
import { DateTimePicker } from "~/components/ui/date";

interface EditHealthWorkerButtonProps {
  healthWorker: {
    id: string;
    user: {
      ci: string | null;
...
export function EditHealthWorkerButton({
  healthWorker,
}: EditHealthWorkerButtonProps) {
  const [open, setOpen] = useState(false);
  const [specialityInput, setSpecialityInput] = useState("");

  const form = useForm<UpdateHealthWorkerSchema>({
    resolver: zodResolver(updateHealthWorkerSchema),
    defaultValues: {
      healthWorkerId: healthWorker.id,
      firstName: healthWorker.user.firstName ?? "",
      lastName: healthWorker.user.lastName ?? "",
      ci: healthWorker.user.ci ?? "",
      email: healthWorker.user.email,
      phone: healthWorker.user.phone ?? "",
      address: healthWorker.user.address ?? "",
      dateOfBirth: healthWorker.user.dateOfBirth ?? undefined,
      specialities: healthWorker.healthWorkerSpecialities.map(
        (s) => s.speciality.name,
      ),
    },
  });

  const { execute, isExecuting } = useAction(updateHealthWorkerAction, {
    onSuccess: () => {
      toast.success("Profesional de salud actualizado exitosamente");
      setOpen(false);
    },
    onError: ({ error }) => {
      toast.error(
        error.serverError ?? "Error al actualizar el profesional de salud",
      );
    },
  });

  const onSubmit = (data: UpdateHealthWorkerSchema) => {
    execute(data);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Editar Profesional de Salud</DialogTitle>
          <DialogDescription>
            Modifica los datos del profesional de salud.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
